# =========================================================
# Batch SDM (Spatial Durbin Model) workflow
# KNN = 4 and 8, with automatic model comparison and output saving
# =========================================================
...
# ---------------------------
# 2) Define SDM function for each KNN setting
# ---------------------------
run_sdm_for_k <- function(k, gdf, target_var, x_vars) {
  cat("\n============================\n")
  cat("Running SDM with KNN =", k, "\n")
  cat("============================\n")
  ...
  cat("Spatial weight matrix created and saved to:", gal_path, "\n")
  ...
  px <- length(x_vars)
  k_eff <- 2 * px + 3  # Approximation: intercept + rho + sigma^2 + X and WX
  ...
  cat("\n===== Model fit statistics for KNN =", k, "=====\n")
  cat(sprintf("R2 (corr^2)      : %.5f\n", R2_corr))
  cat(sprintf("R2 (1 - SSE/SST) : %.5f\n", R2_sse))
  cat(sprintf("Adjusted R2      : %.5f  [k=%d]\n", Adj_R2, k_eff))
...
cat("Result table saved: tables/sdm_knn_results_compare.csv\n")
